...
1. Загрузитесь в рекавери (Recovery).
2. Выполните сброс настроек: `Wipe` ➔ `Format Data` (введите `yes`).
3. Установите прошивку через `Apply Update` ➔ `ADB Sideload` или выберите `.zip` файл из памяти устройства:
   ```bash
   adb sideload DerpFest-13-Tango.zip
